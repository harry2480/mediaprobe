...
export const RawDataViewer: React.FC<Props> = ({ file }) => {
  ...
  return (
    <div className="flex flex-col space-y-4 w-full">
      <h3 className="text-lg font-bold text-zinc-100">RAW Preview & Analysis</h3>

      {error && (
        <div className="p-4 border-l-4 border-yellow-500 bg-yellow-500/10 text-yellow-400 rounded text-sm">
          {error}
        </div>
      )}
      ...
      {!loading && !error && pixelData && (
        <div className="flex flex-col items-center p-4 border border-zinc-700 rounded-lg bg-zinc-800 w-full">
          <p className="text-xs text-zinc-400 mb-2 uppercase">Full RAW Data ({pixelData.width}x{pixelData.height})</p>
          <div className="w-full flex justify-center bg-zinc-900 rounded p-2">
            <canvas
              ref={canvasRef}
